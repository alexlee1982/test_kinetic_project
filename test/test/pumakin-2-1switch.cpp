                      ...
                      KINEMATICS_INVERSE_FLAGS * iflags)
{
	   ...
	   ny = c6*(c5*(c4*(c1*s3 - c2*c3*s1) + s4*(c1*c3 + c2*s1*s3)) + s5*(c4*(c1*c3 + c2*s1*s3) - s4*(c1*s3 - c2*c3*s1))) + s1*s2*s6;
	   nz = -c6*(c5*(c3*c4*s2 - s2*s3*s4) - s5*(c3*s2*s4 + c4*s2*s3)) - c2*s6;


	   ox = -s6*(c5*(c4*(s1*s3 + c1*c2*c3) + s4*(c3*s1 - c1*c2*s3)) + s5*(c4*(c3*s1 - c1*c2*s3) - s4*(s1*s3 + c1*c2*c3))) - c1*c6*s2;
	   oy = c6*s1*s2 - s6*(c5*(c4*(c1*s3 - c2*c3*s1) + s4*(c1*c3 + c2*s1*s3)) + s5*(c4*(c1*c3 + c2*s1*s3) - s4*(c1*s3 - c2*c3*s1)));
	   oz = s6*(c5*(c3*c4*s2 - s2*s3*s4) - s5*(c3*s2*s4 + c4*s2*s3)) - c2*c6;

	   ax = c5*(c4*(c3*s1 - c1*c2*s3) - s4*(s1*s3 + c1*c2*c3)) - s5*(c4*(s1*s3 + c1*c2*c3) + s4*(c3*s1 - c1*c2*s3));
	   ay = c5*(c4*(c1*c3 + c2*s1*s3) - s4*(c1*s3 - c2*c3*s1)) - s5*(c4*(c1*s3 - c2*c3*s1) + s4*(c1*c3 + c2*s1*s3));
	   az = c5*(c3*s2*s4 + c4*s2*s3) + s5*(c3*c4*s2 - s2*s3*s4);


	   px = L5*(c5*(c4*(c3*s1 - c1*c2*s3) - s4*(s1*s3 + c1*c2*c3)) - s5*(c4*(s1*s3 + c1*c2*c3) + s4*(c3*s1 - c1*c2*s3))) + L3*(s1*s3 + c1*c2*c3) + L4*(c4*(s1*s3 + c1*c2*c3) + s4*(c3*s1 - c1*c2*s3)) + L2*c1*c2;
	   py = L5*(c5*(c4*(c1*c3 + c2*s1*s3) - s4*(c1*s3 - c2*c3*s1)) - s5*(c4*(c1*s3 - c2*c3*s1) + s4*(c1*c3 + c2*s1*s3))) + L3*(c1*s3 - c2*c3*s1) + L4*(c4*(c1*s3 - c2*c3*s1) + s4*(c1*c3 + c2*s1*s3)) - L2*c2*s1;
	   pz = L5*(c5*(c3*s2*s4 + c4*s2*s3) + s5*(c3*c4*s2 - s2*s3*s4)) - L2*s2 - L4*(c3*c4*s2 - s2*s3*s4) - L3*c3*s2;
	...
	int  flag)
{
	...
	double a11, a12, a21, a22, b1, b2, A11, A12, A21, A22, B1, B2;
	...
	TH1[0] = atan2(az*px- pz*ax, az*py - pz*ay);
	TH1[1] = atan2(pz*ax - az*px, pz*ay - az*py);
	...
	for (int i = 0; i < 2; i++)
	{
		...
		s1 = sin(th1);
		c1 = cos(th1);
		if ((fabs(az) < DBL_MIN) && (fabs(ay*s1 - ax*c1) < DBL_MIN))
		{
			if ((fabs(pz) < DBL_MIN) && (fabs(py*s1 - px*c1) < DBL_MIN))
			{
				return -1;
			}
			else
			{
				TH2[0] = atan2(pz, py*s1 - px*c1);
				TH2[1] = atan2(-1 * pz, px*c1 - py*s1);
			}
		}
		else
		{
			TH2[0] = atan2(az,ay*s1 - ax*c1);
		...
		for (int j = 0; j < 2; j++)
		{
			...
			s2 = sin(th2);
			c2 = cos(th2);
			th6 = atan2(ny*s1*s2 - nx*c1*s2 - nz*c2, oy*s1*s2 - ox*c1*s2 - oz*c2);
			s6 = sin(th6);
			c6 = cos(th6);
			A = L5*(az*s2 - ax*c1*c2 + ay*c2*s1) - L2 - pz*s2 + px*c1*c2 - py*c2*s1;
			B = py*c1 - L5*(ay*c1 + ax*s1) + px*s1;
			c4 = (A*A + B*B - L3*L3 - L4*L4) / (2 * L3*L4);
			if ((c4 - 1) > 0 && (c4 - 1) < DBL_MIN)
			{
				c4 = 1;
			}
			else if((c4 + 1) < 0 && (c4 + 1) < -1*DBL_MIN)
			{
				c4 = -1;
			}
			...
			for (int k = 0; k < 2; k++)
			{
				...
				s4 = sin(th4);
				c4 = cos(th4);
				a11 = c1*py + px*s1 - L5*(ay*c1 + ax*s1);
				a12 = L5*(az*s2 - ax*c1*c2 + ay*c2*s1) - pz*s2 - L2 + c1*c2*px - c2*py*s1;
				a21 = L2 + pz*s2 - L5*(az*s2 - ax*c1*c2 + ay*c2*s1) - c1*c2*px + c2*py*s1;
				a22 = c1*py + px*s1 - L5*(ay*c1 + ax*s1);
				...
				s3 = (A22*B1) / (A11*A22 - A12*A21) - (A12*B2) / (A11*A22 - A12*A21);
				c3 = (A11*B2) / (A11*A22 - A12*A21) - (A21*B1) / (A11*A22 - A12*A21);

				th3 = atan2(s3, c3);

				s5 = c6*(nx*(s1*(c3*c4 - s3*s4) - c1*c2*(c3*s4 + c4*s3)) + ny*(c1*(c3*c4 - s3*s4) + c2*s1*(c3*s4 + c4*s3)) + nz*s2*(c3*s4 + c4*s3)) - s6*(ox*(s1*(c3*c4 - s3*s4) - c1*c2*(c3*s4 + c4*s3)) + oy*(c1*(c3*c4 - s3*s4) + c2*s1*(c3*s4 + c4*s3)) + oz*s2*(c3*s4 + c4*s3));
				c5 = c6*(nx*(s1*(c3*s4 + c4*s3) + c1*c2*(c3*c4 - s3*s4)) + ny*(c1*(c3*s4 + c4*s3) - c2*s1*(c3*c4 - s3*s4)) - nz*s2*(c3*c4 - s3*s4)) - s6*(ox*(s1*(c3*s4 + c4*s3) + c1*c2*(c3*c4 - s3*s4)) + oy*(c1*(c3*s4 + c4*s3) - c2*s1*(c3*c4 - s3*s4)) - oz*s2*(c3*c4 - s3*s4));

				th5 = atan2(s5, c5);
				
				goal[i * 4 + j * 2 + k][0] = TH1[i] * 180 / PM_PI -180;
				goal[i * 4 + j * 2 + k][1] = TH2[j] * 180 / PM_PI -90;
				goal[i * 4 + j * 2 + k][2] = th3 * 180 / PM_PI;
				goal[i * 4 + j * 2 + k][3] = TH4[k] * 180 / PM_PI;
				goal[i * 4 + j * 2 + k][4] = th5 * 180 / PM_PI;
				goal[i * 4 + j * 2 + k][5] = th6 * 180 / PM_PI;

						for (int m = 0; m < 6; m++)
						{
							if (goal[i * 4 + j * 2 + k * 1][m] < -180)
							{
								goal[i * 4 + j * 2 + k * 1][m] += 360.0;
							}
						}
